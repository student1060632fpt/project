...
class AdminQuanLyLichChieu extends Component {
  constructor(props) {
    ...
    this.state = {
      columns: [
        { title: 'Mã phim', field: 'maPhim', editable: "onAdd" },
        { title: 'Tên phim', field: 'tenPhim' },
        { title: 'Bí danh', field: 'biDanh' },
        { 
          title: 'Trailer', 
          field: 'trailer' ,
          render: rowData=> (<iframe width="200" height="100" src={rowData.trailer} frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>)
        },
        {
          title: 'Hình ảnh',
          field: 'hinhAnh',
  ...
  handleAdd = movie => {
    ...
    movie.ngayKhoiChieu = moment(movie.ngayKhoiChieu).format("L");
    delete movie['tableData'];
    console.log(movie);
    this.props.addLC(movie)
  }
          ...
          editable={{
            ...
            onRowDelete: (oldData) =>
              new Promise((resolve) => {
                setTimeout(() => {
                  resolve();
                  this.props.deleteLC(oldData.maPhim);
                  this.setState((prevState) => {
                    const data = [...prevState.data];
...
const mapStateToProps = state => {
  return {
    list: state.movieReducer.listMovie || [{
      ...
      trailer: "string",
      hinhAnh: "http://movie0706.cybersoft.edu.vn/hinhanh/stringhelo2321312asdasda-adasd_GP09.jpeg?alt=media",
      moTa: "string",
      maNhom: "GP09",
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    editLC: data => {
      dispatch(action.actEditLC(data))
    },
    addLC: data => {
      dispatch(action.actAddLC(data))
    },
    deleteLC: data => {
        dispatch(action.actDeleteLC(data))
    }
